...
def pad_to_size(feat, desire_h, desire_w):
    """ zero-padding a four dim feature matrix: N*C*H*W so that the new Height and Width are the desired ones
        desire_h and desire_w should be larger than the current height and weight
    """
    ...
    for ix in range(0, N):
        ...
        for keys in feat_map_keys:
            ...
            for j in range(0, M):
                ...
                if j == 0:
                    ...
                    if examples_features.shape[2] != max_h or examples_features.shape[3] != max_w:
                        # Upsampling the examples features
            ...
            for scale in exemplar_scales:
                ...
                # use original size if scaled size is too small or too large
                if h1 < 1 or h1 >= image_features.shape[2]:
                    h1 = h
                if w1 < 1 or w1 >= image_features.shape[3]:
                    w1 = w
